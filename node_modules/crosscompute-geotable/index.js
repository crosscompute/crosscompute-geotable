...
function draw_map(k, v) {
  ...
  var map_id = k + '-map',
    table_id = k + '-table';
  var map = L.mapbox.map(map_id).addLayer(L.mapbox.styleLayer(
    'mapbox://styles/mapbox/' + get_geotable_adjective(k) + '-v11'));
  var features = [], feature_packs = v[0], properties = v[1];
  ...
  for (var i = 0; i < feature_packs.length; i++) {
    ...
    var feature,
      ...
      is_line = geometry_type_id == 2 || geometry_type_id == 5,
      d = $.extend({
        'color': 'black',
        'weight': is_line ? 5 : 2,
        'opacity': 0.7,
    ...
    switch(geometry_type_id) {
      ...
      case 4:
        for (var j = 0; j < geometry_coordinates.length; j++) {
          feature = L.circleMarker(geometry_coordinates[j], d);
          d.radius_in_pixels && feature.setRadius(+d.radius_in_pixels);
          add_feature(features, feature, i, table_id);
        }
        break;
